...
function capture(res) {
  ...
  if (!fs.existsSync(fullpath)) { console.error("Could not find path: " + fullpath); return; }
	mockImages = fs.readdirSync(fullpath);
  if (mockImages) { mockImages = mockImages.filter(isImage).sort(); }

  if (mockImages.length == 0) { console.error("Cound't find any mock images in path: " + imagePath); return; }
  response = res;
